...
def _log(conn, appliance_id, appliance_name, action, reason):
    conn.execute(
        "INNSERT INTO event_log (timestamp, appliance_id, appliance_name, action, reason)"
        "VALUES (?,?,?,?,?)",
        (_now(), appliance_id, appliance_name, action, reason)
    )

def _get_appliacne(conn, appliance_id):
    row = conn.execute(
        "SELECT * FROM appliances WHERE id = ?", (appliance_id,)
    ).fetchone()
    if row is None:
        raise ApplianceNotFound("No appliance with id (appliance_id)")
    return row
...
def _auto_restore(conn, trigger_reason):
    ...
    for appliance in shed:
        if current_load + appliance["wattage"] <= CAPACITY_WATTS:
            conn.execute(
                "UPDATE appliances SET state = 'runnig' WHERE id = '?",
                (appliance["id"],),
            )
...
def register_appliance(conn, name, wattage, priority):
    ...
    appliance_id = cusror.lastrowid
    _log(conn, appliance_id, name, "registered", f"{name} regsitered ({wattage}W, priority {priority})")

    return appliance_id

def delete_appliance(conn, appliance_id):
    appliance = _get_appliacne(conn, appliance_id)
    ...
    for c in candidates:
        ...
        if current_load - freed + appliance["wattage"] > CAPACITY_WATTS:
            raise CapacityRejected(
                f"Cannot turn on {appliance["name"]}: "
                f"only {CAPACITY_WATTS - (current_load-freed)}W would be availabe"
...
def turn_off(conn, appliance_id):
    ...
    if appliance["state"] == "shed":
        conn.execute("UPDATE appliance SET state = 'off' WHERE id = ?", (appliance_id,))
